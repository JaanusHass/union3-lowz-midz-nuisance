...
$0.52$ after the nuisance mode is included, with the CPL+nuisance posterior shifting
...
individual-SN data were not publicly available for direct analysis. Cort\^es \&
...
(AIC $-3.48$), while the CPL$+$nuisance mean shifts to $w_0=-0.999$, $w_a=-0.192$---close
...
tracing a true dark-energy signal would be expected to retain its amplitude. Instead
...
CPL $\Delta\chi^2$ [a] & $\approx7.52$ & $\approx4.74$ & $\approx4.74$ \\
CPL AIC gain [a] & $\approx+3.52$ & $\approx+0.74$ & $\approx+0.74$ \\
...
CPL after nuisance AIC & $-3.13$ & $-2.83$ & $-2.05$ \\
...
reflects genuine residual structure rather than overfitting: the random smooth-template
null test of Section~7.2 bounds the overfitting contribution (random templates reach at
most $\Delta\chi^2=12.4<17.1$), and the quantity relevant to the dark-energy claim is the
$0.87$ that CPL adds after $T$, not the $10.45$.
...
$0.87$ after $T$, but $T$ adds $10.45$ after CPL---CPL lives inside $T$, while $T$ reaches
well beyond CPL.}
\label{fig:overlap}
\end{figure}
...
compare $\Lambda$CDM$+$nuisance, CPL, and CPL$+$nuisance. If CPL loses AIC support after
the nuisance is included, the Union3 contribution to the $w_0$--$w_a$ preference is
systematics-limited. If the preference persists, the concern is resolved. The
...
\bibitem{efstathiou2025} Efstathiou G.\ 2025, MNRAS --- Systematic offset in low-$z$ supernovae in Pantheon$+$ and DES5Y.
