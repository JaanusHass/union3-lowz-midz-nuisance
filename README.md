...
+ Union3) and the full-likelihood run (Planck NPIPE CamSpec + DESI DR1 BAO + Union3) — are
reproducible from the Cobaya configuration in `cobaya/`, given an external Cobaya + CAMB
installation and the Planck/DESI/Union3 likelihood data (installable with `cobaya-install`;
see `cobaya/README.md`). The MCMC chains themselves are large and are not committed
(see `.gitignore`); they are regenerated by the runs. The joint runs use the published
numbers' best-sample chi2 as an MCMC proxy and are public cross-checks rather than the
official, internally validated DESI+CMB+Union3 likelihood.
