...
Expected qualitative result:
    - 200 random templates
    - 0/200 random templates exceed the original template improvement
    - empirical tail bound p < 1/201
...
def main() -> None:
    ...
    original_delta, original_amp = nuisance_delta_chi2(mu_obs, precision, mu_model, primary_template(z))
        ...
        rows.append(
            {
                ...
                **meta,
                "delta_chi2": delta,
                "amplitude": amp,
                "exceeds_original": bool(delta >= original_delta),
            }
        )

    df = pd.DataFrame(rows)
    n_exceed = int(df["exceeds_original"].sum())
        ...
        [
            {
                ...
                "seed": args.seed,
                "original_delta_chi2": original_delta,
                "original_amplitude": original_amp,
                "random_median_delta_chi2": df["delta_chi2"].median(),
                "random_95th_percentile_delta_chi2": df["delta_chi2"].quantile(0.95),
                "random_max_delta_chi2": df["delta_chi2"].max(),
                "n_exceeded_original": n_exceed,
                "empirical_tail_bound": f"{n_exceed}/{args.n_random}; p < 1/{args.n_random + 1}" if n_exceed == 0 else f"{n_exceed}/{args.n_random}",
            }
        ]
    )

    summary_path = args.outdir / "random_template_null_summary.csv"
