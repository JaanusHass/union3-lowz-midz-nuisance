...
def fit_cpl(
    ...
    """
    Fit CPL parameters w0, wa at fixed Omega_m.

    The intercept M and optional nuisance amplitude are solved analytically
    by generalized least squares for each trial (w0, wa).
    ...
    for start in starts:
        # First use Nelder-Mead only as an unconstrained search
        ...
        # Clip the Nelder-Mead result back into the allowed range.
        clipped = np.array(
            [
        ...
        # Final accepted result must come from bounded L-BFGS-B.
    ...
    return FitResult(
        "CPL" if template is None else "CPL+nuisance",
        chi2,
        params,
        2 if template is None else 3,
    )
